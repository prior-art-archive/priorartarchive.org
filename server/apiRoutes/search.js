...
app.post('/api/search', (req, res)=> {
	...
	if (query && operatorSet.has(operator)) {
		lambda.invoke({
			...
			new Promise(async (resolve, reject) => {
				if (data && data.StatusCode === 200) {
					...
					if (fileType.length || source.length) {
						...
						if (source.length) {
							const orgs = await Promise.all(source.map(getOrganizationBySlug)).catch(reject);
							if (!orgs) return;
							payload.query.bool.filter.push({
									...
									if (Array.isArray(term[span].clauses)) {
										term[span].clauses.forEach((clause, j) => {
											if (typeof clause.term === 'object') {
												payload.query.bool.must[i][span].clauses[j].span_term = clause.term;
												delete payload.query.bool.must[i][span].clauses[j].term;
											}
										});
									}
								}
			...
			}).then(result => res.status(200).json(result)).catch(error => res.status(400).json(error));
		});
	} else {
